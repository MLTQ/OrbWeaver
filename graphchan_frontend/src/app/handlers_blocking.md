...
- `handle_peer_blocks_exported` - Stores CSV export text in `blocking_state.peer_export_text` for inline display
- `handle_peer_blocks_imported` - Clears import text, refreshes blocked peers list
...
- `handle_ip_blocks_exported` - Stores export text in `blocking_state.ip_export_text` for inline display
...
|-----------|---------|------------------|
| `messages.rs` | All 19 handler methods exist on `GraphchanApp` | Method removal/rename |
| `spawners.rs` | Handlers call `spawn_load_ip_blocks`, `spawn_load_ip_block_stats`, `spawn_load_blocked_peers` | Spawner removal |
...
- Export text shown inline in the UI (selectable text area) rather than clipboard/file dialog
