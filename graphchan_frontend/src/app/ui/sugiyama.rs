...
pub fn build_sugiyama_layout(
    posts: &[PostView],
    sizes: &HashMap<String, egui::Vec2>,
...
pub fn render_sugiyama(app: &mut GraphchanApp, ui: &mut egui::Ui, state: &mut ThreadState) {
    ...
    for post in &posts {
        if let Some(node) = state.sugiyama_nodes.get(&post.id) {
            ...
            let screen_center =
                egui::pos2(0.0, 0.0) + center_offset + (node.pos.to_vec2() * state.graph_zoom);
            ...
            layouts.push(NodeLayoutData {
                post: post.clone(),
                rect: rect_node,
                attachments: if !post.files.is_empty() {
                    Some(post.files.clone())
                } else {
                    None
                },
    ...
    for layout in &layouts {
        for parent_id in &layout.post.parent_post_ids {
            if let Some(parent_layout) = layouts.iter().find(|l| &l.post.id == parent_id) {
                ...
                let is_selected_edge = state.selected_post.as_ref() == Some(&layout.post.id)
                    || state.selected_post.as_ref() == Some(parent_id);
                let color = if is_selected_edge {
                    Color32::from_rgb(250, 208, 108)
                } else {
                    Color32::from_rgb(100, 150, 200)
                };
                let stroke_width = if is_selected_edge {
                    2.5 * state.graph_zoom
                } else {
                    1.5 * state.graph_zoom
                };
