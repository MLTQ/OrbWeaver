...
impl GraphchanApp {
    ...
    fn render_reactions(&mut self, ui: &mut egui::Ui, state: &mut ThreadState, post_id: &str) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            if ui
                .small_button(if picker_open { "−" } else { "+" })
                .clicked()
            {
                if picker_open {
    ...
    pub(crate) fn render_thread(
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                let button =
                    egui::Button::new(RichText::new(button_text).color(button_color)).frame(true);
                ...
                if state.is_hosting {
                    ui.label(
                        RichText::new("Broadcasting thread to peers")
                            .small()
                            .color(Color32::GRAY),
                    );
                } else {
                    ui.label(
                        RichText::new("Not broadcasting (lurking)")
                            .small()
                            .color(Color32::GRAY),
                    );
                }
                ...
                if state.summary.source_url.is_some() {
                    ...
                    if state.refreshing_source {
                        ...
                        if let Some(last) = &state.summary.last_refreshed_at {
                            ui.label(
                                RichText::new(format!("Last: {}", format_timestamp(last)))
                                    .small()
                                    .color(Color32::GRAY),
                            );
                        }
        ...
        if state.display_mode == ThreadDisplayMode::Graph {
            ...
            if state.graph_nodes.is_empty() {
                if let Some(details) = &state.details {
                    state.graph_nodes = chronological::build_chronological_layout(
                        &details.posts,
                        state.time_bin_seconds,
                    );
                    state.graph_zoom = 1.0;
                    state.graph_offset = egui::vec2(0.0, 0.0);
                }
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for post in &posts_clone {
                            ...
                            let border_color =
                                if let Some(reactions_response) = state.reactions.get(&post.id) {
                                    super::reaction_colors::calculate_post_color(
                                        &reactions_response.counts,
                                        max_score,
                                    )
                                } else {
                            ...
                            let response = frame.show(ui, |ui| {
                                ...
                                if let Some(author) = &post.author_peer_id {
                                    ...
                                    ui.horizontal(|ui| {
                                        let avatar_id =
                                            peer.as_ref().and_then(|p| p.avatar_file_id.clone());
                                        if let Some(avatar_id) = avatar_id {
                                            if let Some(texture) =
                                                self.image_textures.get(&avatar_id)
                                            {
                                                ui.add(
                                                ...
                                                self.image_pending.remove(&avatar_id)
                                            {
                                                let color =
                                                ...
                                                self.image_textures
                                                    .insert(avatar_id.clone(), tex.clone());
                                                ui.add(
                                                ...
                                                self.image_errors.get(&avatar_id)
                                            {
                                                ui.colored_label(egui::Color32::RED, "Error");
                                                ui.label(err);
                                            } else {
                                                let needs_download =
                                                    !self.image_loading.contains(&avatar_id);
                                                if needs_download {
                                                    let url = crate::app::resolve_blob_url(
                                                        &self.base_url_input,
                                                        &avatar_id,
                                                    );
                                        ...
                                        if let Some(metadata) = &post.metadata {
                                            if let Some(agent) = &metadata.agent {
                                                ui.add_space(4.0);
                                                let badge_text = if let Some(version) =
                                                    &agent.version
                                                {
                                ...
                                if !post.parent_post_ids.is_empty() {
                                    ...
                                    ui.horizontal_wrapped(|ui| {
                                        ui.label(
                                            RichText::new("↩ Replying to:").color(Color32::GRAY),
                                        );
                                        for parent_id in &post.parent_post_ids {
                                            let short_id = if parent_id.len() > 8 {
                                                &parent_id[..8]
                                            } else {
                                                parent_id
                                            };
                                ...
                                if let Some(children) = children_map.get(&post.id) {
                                    if !children.is_empty() {
                                        ...
                                        ui.horizontal_wrapped(|ui| {
                                            ui.label(
                                                RichText::new("↪ Replies:").color(Color32::GRAY),
                                            );
                                            for child_id in children {
                                                let short_id = if child_id.len() > 8 {
                                                    &child_id[..8]
                                                } else {
                                                    child_id
                                                };
