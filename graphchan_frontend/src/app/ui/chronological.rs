...
use super::input;
use super::node::{estimate_node_height, is_image, render_node, NodeLayoutData};
...
pub fn build_chronological_layout(
    posts: &[PostView],
    bin_seconds: i64,
    ...
    let post_lookup: HashMap<String, &PostView> = posts.iter().map(|p| (p.id.clone(), p)).collect();
...
fn create_time_bins(
    timestamped_posts: &[(DateTime<Utc>, &PostView)],
    bin_seconds: i64,
) -> Vec<ChronoBin> {
        ...
        ui.allocate_ui_at_rect(control_rect, |ui| {
            ui.push_id("chronological_controls", |ui| {
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add(
                            egui::Slider::new(&mut bin_minutes, 1.0..=1440.0)
                                .logarithmic(true)
                    ...
                    if rebuild_layout {
                        let posts = state
                            ...
                            .map(|d| d.posts.clone())
                            .unwrap_or_default();
                        state.chronological_nodes =
                            build_chronological_layout(&posts, state.time_bin_seconds);
                    }
...
fn draw_orthogonal_edges(
    ...
    for layout in layouts {
        for parent_id in &layout.post.parent_post_ids {
            if let Some(parent_rect) = rect_lookup.get(parent_id) {
                ...
                let is_hovered =
                    hovered_post == Some(&layout.post.id) || hovered_post == Some(parent_id);
                ...
                let width = if is_reply_target || is_selected || is_parent_selected {
                    2.5
                } else if is_hovered {
                    2.0
                } else {
    ...
    for (_, indices) in outgoing.iter_mut() {
        indices.sort_by(|&a, &b| {
            edges[a]
                ...
                .center()
                .x
                .partial_cmp(&edges[b].child_rect.center().x)
    ...
    for (_, indices) in incoming.iter_mut() {
        indices.sort_by(|&a, &b| {
            edges[a]
                ...
                .center()
                .x
                .partial_cmp(&edges[b].parent_rect.center().x)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
    }
    ...
    for (_, indices) in groups.iter_mut() {
        ...
        for (i, &edge_idx) in indices.iter().enumerate() {
            ...
            let mid_y = (center_y + offset)
                .min(end_y - 20.0 * state.graph_zoom)
                .max(start_y + 20.0 * state.graph_zoom);
            ...
            painter.line_segment([end, end + egui::vec2(-arrow_size, -arrow_size)], stroke);
            painter.line_segment([end, end + egui::vec2(arrow_size, -arrow_size)], stroke);
        }
    }
}
...
fn draw_time_axis(
    ...
    painter.line_segment(
        [
            egui::pos2(line_x, rect.top() + 20.0),
            egui::pos2(line_x, rect.bottom() - 20.0),
        ],
