...
pub fn render_node(
    ...
    let reaction_color = if let Some(reactions_response) = state.reactions.get(&layout.post.id) {
        let max_score =
            super::reaction_colors::calculate_max_score_from_responses(&state.reactions);
            ...
            |ui| {
                ...
                let inner_response = egui::Frame::none()
                    ...
                    .stroke(egui::Stroke::new(
                        (if selected || is_neighbor || is_secondary {
                            2.5
                        } else {
                            1.5
                        }) * zoom,
                        ...
                        ui.vertical(|ui| {
                            ...
                            if !layout.post.parent_post_ids.is_empty() {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    ...
                                    for (i, parent_id) in
                                        layout.post.parent_post_ids.iter().enumerate()
                                    {
                                        let is_parent_cursor =
                                            selected && state.parent_cursor_index == i;
                                        render_post_link(
                                            ui,
                                            state,
                                            parent_id,
                            ...
                            if !children.is_empty() {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    ui.label(
                                        RichText::new("↪ Replies:")
                                            .size(11.0 * zoom)
                                            .color(Color32::GRAY),
                                    );
                                    for (i, child_id) in children.iter().enumerate() {
                                        let is_reply_cursor =
                                            selected && state.reply_cursor_index == i;
                                        render_post_link(
                                            ui,
                                            state,
                                            child_id,
        ...
        if let Some(author_id) = &post.author_peer_id {
            ...
            if let Some(avatar_id) = peer.as_ref().and_then(|p| p.avatar_file_id.clone()) {
                if let Some(texture) = app.image_textures.get(&avatar_id) {
                    ...
                    let color =
                        egui::ColorImage::from_rgba_unmultiplied(pending.size, &pending.pixels);
                    let tex =
                        ui.ctx()
                            .load_texture(&avatar_id, color, egui::TextureOptions::default());
                ...
                } else if !app.image_loading.contains(&avatar_id)
                    && !app.image_errors.contains_key(&avatar_id)
                {
            ...
            if ui
                .add(egui::Link::new(
                    RichText::new(name).strong().size(12.0 * zoom),
                ))
                .clicked()
            {
...
fn render_post_link(
    ...
    let short_id = if target_id.len() > 8 {
        &target_id[..8]
    } else {
        target_id
    };
        ...
        if let Some(details) = &state.details {
            if let Some(post) = details.posts.iter().find(|p| p.id == target_id) {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        RichText::new(format_timestamp(&post.created_at)).color(Color32::GRAY),
                    );
                });
                ui.separator();
...
fn render_node_actions(
    ...
    ui.horizontal(|ui| {
        if ui
            .button(RichText::new("↩ Reply").size(13.0 * zoom))
            .clicked()
        {
            GraphchanApp::set_reply_target(state, &post.id);
        }
        if ui
            .button(RichText::new("❝ Quote").size(13.0 * zoom))
            .clicked()
        {
...
fn render_node_reactions(
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        if let Some(reactions_response) = state.reactions.get(post_id) {
            for (emoji, count) in &reactions_response.counts {
                ...
                let button = ui.add(egui::Button::new(
                    RichText::new(button_text).size(12.0 * zoom),
                ));
                if button.clicked() {
                    ...
                    if let Some(self_id) = self_peer_id {
                        let has_reacted = reactions_response
                            .reactions
                            .iter()
        ...
        let button = ui.add(egui::Button::new(
            RichText::new(if picker_open { "−" } else { "+" }).size(12.0 * zoom),
        ));
        if button.clicked() {
            if picker_open {
        ...
        if picker_open {
            ...
            for emoji in quick_emojis {
                if ui
                    .add(egui::Button::new(RichText::new(emoji).size(12.0 * zoom)))
                    .clicked()
                {
                    app.spawn_add_reaction(post_id, emoji);
                    state.emoji_picker_open = None;
    ...
    for file in files {
        ...
        if !is_available {
            ui.horizontal(|ui| {
                ...
                ui.label(
                    RichText::new(format!("{} {}{}", icon, name, size_text)).size(11.0 * zoom),
                );
                ...
                match download_status {
                    ...
                    "failed" => {
                        ui.colored_label(
                            ...
                            RichText::new("Failed").size(10.0 * zoom),
                        );
                        if ui
                            .button(RichText::new("↻ Retry").size(10.0 * zoom))
                            .clicked()
                        {
                    ...
                    _ => {
                        // "pending" or unknown
                        if ui
                            .button(RichText::new("⬇ Download").size(10.0 * zoom))
                            .clicked()
                        {
                            // Get thread_id from current view state
                            let thread_id = if let ViewState::Thread(state) = &app.view {
        ...
        if mime.starts_with("image/") {
            match image_preview(app, ui, file, api_base) {
                ...
                ImagePreview::Error(err) => {
                    ui.colored_label(
                        Color32::LIGHT_RED,
                        RichText::new(format!("Image error: {err}")).size(11.0 * zoom),
                    );
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button(RichText::new(icon).size(14.0 * zoom)).clicked() {
                    if let ViewState::Thread(state) = &mut app.view {
                        if is_playing {
                            ...
                            state.audio_promise = Some(poll_promise::Promise::spawn_thread(
                                "load_audio",
                                move || {
                ...
                ui.label(
                    RichText::new(file.original_name.as_deref().unwrap_or("audio"))
                        .size(12.0 * zoom),
                );
            });
        } else {
